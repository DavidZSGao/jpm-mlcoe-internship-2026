...
"""Particle filtering with invertible particle flow proposals (Li, 2017)."""
...
from mlcoe_q2.models.flows.base import ParticleFlow, ParticleFlowResult

LogLikelihoodFn = Callable[[tf.Tensor, tf.Tensor, Optional[tf.Tensor]], tf.Tensor]


@dataclass
class ParticleFlowParticleFilterResult:
    """Outputs produced by the particle-flow particle filter."""
...
def particle_flow_particle_filter(
...
) -> ParticleFlowParticleFilterResult:
    """Sequential importance resampling with invertible particle flow proposals."""
    ...
    if log_likelihood_fn is None:
        ...
        def gaussian_log_likelihood(
            ...
            obs_dim = tf.cast(model.observation_dim, tf.float32)
            log_two_pi = tf.math.log(tf.constant(2.0 * 3.141592653589793, dtype=tf.float32))
            return -0.5 * (maha + log_det + obs_dim * log_two_pi)
    ...
    for t in tf.range(num_timesteps):
        ...
        flow_result: ParticleFlowResult = flow(
            model=model,
            particles=propagated_particles,
            weights=_normalize_log_weights(current_log_weights),
            observation=obs_t,
            control=control_t,
        )
        flow_diagnostics.append(flow_result.diagnostics)

        transformed_particles = flow_result.propagated_particles
        log_det_jac = tf.convert_to_tensor(
            flow_result.log_jacobians,
            dtype=tf.float32,
        )
        log_jacobian_ta = log_jacobian_ta.write(t, log_det_jac)
        ...
        log_weights = current_log_weights + log_update - log_det_jac
        ...
        should_resample = tf.math.logical_and(
            threshold > 0.0,
            effective_sample_size <= threshold * particle_count_f,
        )
        ...
        post_particles = tf.cond(
            should_resample,
            lambda: tf.gather(transformed_particles, resampled_indices),
            lambda: transformed_particles,
        )
    ...
    return ParticleFlowParticleFilterResult(
