...
from mlcoe_q2.models.resampling.sinkhorn import entropy_regularized_transport
...
def differentiable_particle_filter(
    ...
    neural_resampler: Optional[NeuralOTResampler] = None,
    ...
    if resampling_method == "neural_ot":
        diagnostics["neural_transport_error"] = tf.TensorArray(
            dtype=tf.float32,
            size=flow_steps_size,
        )
    ...
    for t in time_indices:
        ...
        if resampling_method == "ot":
            transport_plan = entropy_regularized_transport(
                mixed_weights,
                propagated_particles,
                epsilon_value,
                ot_num_iters,
            ...
            resampled_weights = uniform_weights
            resampled_log_weights = tf.math.log(resampled_weights)
        elif resampling_method == "neural_ot":
            if neural_resampler is None:
                raise ValueError(
                    "neural_resampler must be provided when using 'neural_ot'"
                )
            transport_plan, approx_error = neural_resampler.predict_transport(
                mixed_weights,
                propagated_particles,
                epsilon_value,
            )
            if "neural_transport_error" in diagnostics:
                error_value = (
                    approx_error if approx_error is not None else tf.constant(0.0)
                )
                diagnostics["neural_transport_error"] = diagnostics[
                    "neural_transport_error"
                ].write(index, error_value)
            resampled_particles = tf.matmul(transport_plan, propagated_particles)
            resampled_weights = uniform_weights
            resampled_log_weights = tf.math.log(resampled_weights)
